...
class RasterService {
      ...
      for (let i = imageCount - 1; i >= 1; i--) {
        try {
          ...
          if (omem <= MAX_SAFE_MEMORY_MB && ow <= MAX_OUTPUT_DIM * 2 && oh <= MAX_OUTPUT_DIM * 2) {
            ...
            const dataUrl = this.renderToCanvas(rasterData, outW, outH, bandCount, noDataValue, palette, renderingMode, dataMin, dataMax);
    ...
    for (let ty = 0; ty < tilesY; ty++) {
      for (let tx = 0; tx < tilesX; tx++) {
        ...
        try {
          ...
          this.fillImageData(tileImgData, tileData, bandCount, noDataValue, palette, renderingMode, dataMin, dataMax);
   ...
   */
  fillImageData(imageData, rasterData, bandCount, noDataValue, palette = null, renderingMode = 'grayscale', dataMin = undefined, dataMax = undefined) {
    ...
    if (renderingMode === 'palette' && palette) {
      for (let i = 0; i < pixelCount; i++) {
        const v = bandR[i];
        if (v === noDataValue || isNaN(v) || !isFinite(v) || v < -9000) {
   ...
   */
  renderToCanvas(rasterData, width, height, bandCount, noDataValue, palette = null, renderingMode = 'grayscale', dataMin = undefined, dataMax = undefined) {
    const canvas = document.createElement('canvas');
    canvas.width = width;
    ...
    this.fillImageData(imageData, rasterData, bandCount, noDataValue, palette, renderingMode, dataMin, dataMax);
